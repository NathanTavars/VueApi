  ...
  <div class="h-[calc(100vh-50px)] bg-indigo-500 px-4">
        <div class="rounded-md p-[20px] h-full">
          <div class="w-full flex justify-between py-5">
            <h3 class="text-5xl font-bold text-left text-white">Roupas</h3>
            ...
            <div v-if="modalOpen" class="fixed inset-0 flex items-center justify-center bg-gray-500 bg-opacity-50 top-0">
                <div class="absolute top-5 right-16 left-96 bg-white p-8 rounded-lg shadow">
                  <div class="flex mb-2">
                    <p class="flex w-full font-bold text-2xl">Criar uma nova Roupa</p>
                  ...
                  <form class="mb-6" @submit.prevent="salvar">
                    <label for="id" class="block text-sm font-medium text-gray-700">ID da Roupa</label>
                    <input v-model="form.roupaId" type="number" class="mt-1 p-2 border border-gray-300 rounded-md w-full" required >

                    <label for="modelo" class="block mt-4 text-sm font-medium text-gray-700">Modelo</label>
                    <input v-model="form.modelo" type="text" class="mt-1 p-2 border border-gray-300 rounded-md w-full" required>

                    <label for="tamanho" class="block mt-4 text-sm font-medium text-gray-700">Tamanho</label>
                    <input v-model="form.tamanho" type="text" class="mt-1 p-2 border border-gray-300 rounded-md w-full" required>

                    <label for="cor" class="block mt-4 text-sm font-medium text-gray-700">Cor</label>
                    <input v-model="form.cor" type="text" class="mt-1 p-2 border border-gray-300 rounded-md w-full" required>

                    <label for="valor" class="block mt-4 text-sm font-medium text-gray-700">Valor</label>
          ...
          <div class="overflow-x-auto sm:rounded-lg mt-4">
            <table class="w-full text-sm text-left text-gray-500">
              ...
              <tbody>
                <tr v-for="roupa of roupas" :key="roupa.id" class="bg-white border-b">
                  ...
                  <td class="py-4 px-6 text-right">
                    <button @click="editar(roupa)" class="font-medium text-blue-600 hover:underline">Edit</button>
                  </td>

                  <td class="py-4 px-6 text-right">
                    <button @click="remover(roupa)" class="font-medium text-red-600 hover:underline">Delete</button>
                  </td>
                </tr>
              </tbody>
            </table>
